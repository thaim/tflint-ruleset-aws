...
func NewAwsLambdaProvisionedConcurrencyConfigInvalidFunctionNameRule() *AwsLambdaProvisionedConcurrencyConfigInvalidFunctionNameRule {
	return &AwsLambdaProvisionedConcurrencyConfigInvalidFunctionNameRule{
		...
		max:           140,
		min:           1,
		pattern:       regexp.MustCompile(`^(arn:(aws[a-zA-Z-]*)?:lambda:)?((eusc-)?[a-z]{2}((-gov)|(-iso([a-z]?)))?-[a-z]+-\d{1}:)?(\d{12}:)?(function:)?([a-zA-Z0-9-_]+)(:(\$LATEST|[a-zA-Z0-9-_]+))?$`),
	}
}
	...
	for _, resource := range resources.Blocks {
		...
		err := runner.EvaluateExpr(attribute.Expr, func (val string) error {
			...
			if !r.pattern.MatchString(val) {
				runner.EmitIssue(
					r,
					fmt.Sprintf(`"%s" does not match valid pattern %s`, truncateLongMessage(val), `^(arn:(aws[a-zA-Z-]*)?:lambda:)?((eusc-)?[a-z]{2}((-gov)|(-iso([a-z]?)))?-[a-z]+-\d{1}:)?(\d{12}:)?(function:)?([a-zA-Z0-9-_]+)(:(\$LATEST|[a-zA-Z0-9-_]+))?$`),
					attribute.Expr.Range(),
				)
			}
			return nil
